...
class AvailabilityChart extends ChartComponent {
  ...
  private zoom(event: any, direction: string, xPos: number) {
    if (this.chartOptions.isCompact) return;
    var range = Math.max(
      this.getMinZoomedRange(),
      this.zoomedToMillis - this.zoomedFromMillis
    );
    let percentile =
      (xPos - this.sparkLineChart.x.range()[0]) /
      (this.sparkLineChart.x.range()[1] - this.sparkLineChart.x.range()[0]);
    let leftImpact = percentile * 0.2 * range;
    let rightImpact = (1 - percentile) * 0.2 * range;
    if (direction == 'out') {
      this.zoomedFromMillis = Math.max(
        this.zoomedFromMillis - leftImpact,
        this.fromMillis
      );
      this.zoomedToMillis = Math.min(
        this.zoomedToMillis + rightImpact,
        this.toMillis
      );
    } else {
      let prospectiveFromMillis = Math.max(
        this.zoomedFromMillis + leftImpact,
        this.fromMillis
      );
      let prospectiveToMillis = Math.min(
        this.zoomedToMillis - rightImpact,
        this.toMillis
      );
      if (prospectiveToMillis - prospectiveFromMillis >= this.getMinZoomedRange()) {
        ...
        let offBy =
          this.getMinZoomedRange() - (prospectiveToMillis - prospectiveFromMillis);
  ...
  private createDisplayBuckets(fromMillis: number, toMillis: number) {
    var keysInRange = Object.keys(
      this.transformedAvailability[0].availabilityCount['']
    ).reduce(
      (inRangeObj: any, timestamp: string, i: number, timestamps: Array<string>) => {
        ...
        if (currTSMillis >= fromMillis && currTSMillis <= toMillis) {
          inRangeObj[new Date(currTSMillis).toISOString()] =
            this.transformedAvailability[0].availabilityCount[''][timestamp];
          return inRangeObj;
        }
        if (currTSMillis < fromMillis && nextTSMillis > fromMillis) {
          inRangeObj[new Date(fromMillis).toISOString()] =
            this.transformedAvailability[0].availabilityCount[''][timestamp];
          return inRangeObj;
        }
        return inRangeObj;
    ...
    var rawBucketCount: number = Math.ceil(
      (toMillis - fromMillis) / this.bucketSize
    );
    ...
    var createKey = (millis) =>
      Math.round(Math.floor(millis / computedBucketSize) * computedBucketSize);
    ...
    Object.keys(keysInRange)
      ...
      .forEach((ts: string, i) => {
        ...
        buckets[new Date(computedKey).toISOString()].count +=
          keysInRange[ts].count / bucketMultiplier;
      });

    if (fromMillis !== null && firstBucket !== null) {
      buckets[new Date(fromMillis).toISOString()] =
        buckets[new Date(firstBucket).toISOString()];
    }
    if (toMillis !== null && lastBucket !== null) {
      buckets[new Date(toMillis).toISOString()] =
        buckets[new Date(lastBucket).toISOString()];
    }
  ...
  private setRangeVariables(rawAvailability) {
    if (
      Utils.safeNotNullOrUndefined(
        () =>
          rawAvailability.range.from ||
          rawAvailability.range.to ||
          rawAvailability.intervalSize
      )
    ) {
  ...
  private resizeListener() {
    ...
    this.setTicks();
    this.drawWarmRange();
    if (this.chartOptions.isCompact) this.buildCompactFromAndTo();
    setTimeout(() => {
      this.drawGhost();
    }, 100);
  }
  ...
  public render(
    ...
    this.maxBuckets = this.chartOptions.maxBuckets
      ? this.chartOptions.maxBuckets
      : 500;
    this.setRangeVariables(rawAvailability);

    var startBucket =
      this.fromMillis !== null && this.bucketSize !== null
        ? Math.round(Math.floor(this.fromMillis / this.bucketSize) * this.bucketSize)
        : null;
    var endBucket =
      this.toMillis !== null && this.bucketSize !== null
        ? Math.round(Math.floor(this.toMillis / this.bucketSize) * this.bucketSize)
        : null;

    if (startBucket !== null && startBucket === endBucket) {
      this.fromMillis =
        Math.floor(this.fromMillis / this.bucketSize) * this.bucketSize;
      this.toMillis = this.fromMillis + this.bucketSize;
      this.bucketSize = this.bucketSize / 60;
    }

    this.ae = [
      new this.uxClient.AggregateExpression(
        { predicateString: '' },
        { property: 'Count', type: 'Double' },
        ['count'],
        { from: new Date(this.fromMillis), to: new Date(this.toMillis) },
        null,
        'grey',
        'Availability'
      ),
    ...
    this.chartOptions.brushMoveAction = (from, to) => {
      ...
      if (brushMoveAction != null) brushMoveAction(from, to);
    };
    ...
    if (this.timePickerContainer == null) {
      ...
      this.timePickerContainer = this.targetElement
        .append('div')
        .classed('tsi-timePickerContainer', true);
      this.timePickerChart = this.timePickerContainer
        .append('div')
        .classed('tsi-timePickerChart', true);
      var sparkLineContainer = this.targetElement
        .append('div')
        .classed('tsi-sparklineContainer', true);
      this.timePickerTextContainer = this.targetElement
        .append('div')
        .classed('tsi-timePickerTextContainer', true)
        .style(
          'margin-left',
          this.chartOptions.availabilityLeftMargin + this.margins.left
        );
      this.timePickerLineChart = new LineChart(this.timePickerChart.node() as any);
      this.timePickerLineChart.chartMargins.left =
        this.chartOptions.availabilityLeftMargin - this.margins.left;
      this.buildFromAndToContainer();
      this.sparkLineChart = new LineChart(sparkLineContainer.node() as any);
      this.sparkLineChart.chartMargins.left =
        this.chartOptions.availabilityLeftMargin - this.margins.left;
      this.dateTimePickerContainer = this.targetElement
        .append('div')
        .classed('tsi-dateTimePickerContainer', true);
      this.dateTimePicker = new DateTimePicker(this.dateTimePickerContainer.node());

      window.addEventListener('resize', this.resizeListener);

      var pickerContainerAndContent = this.targetElement.selectAll(
        '.tsi-dateTimePickerContainer, .tsi-dateTimePickerContainer *'
      );
    }
    ...
    if (this.chartOptions.isCompact) {
      ...
      if (!this.chartOptions.persistDateTimeButtonRange)
        this.targetElement
          .select('.tsi-timePickerTextContainer')
          .style('display', 'none');
      this.targetElement.select('.tsi-zoomButtonContainer').style('display', 'none');
      this.targetElement
        .select('.tsi-timePickerContainer')
        .style('max-height', '68px')
        .style('top', this.chartOptions.persistDateTimeButtonRange ? '6px' : '20px');
    } else {
    ...
    var visibileAvailability = this.createDisplayBuckets(
      this.fromMillis,
      this.toMillis
    );
    this.sparkLineChart.render(visibileAvailability, sparkLineOptions, this.ae);

    this.timePickerLineChart.render(
      visibileAvailability,
      this.chartOptions,
      this.ae
    );
    this.setTicks();
    ...
    if (!this.chartOptions.preserveAvailabilityState) {
      ...
      this.zoomedFromMillis = this.chartOptions.defaultAvailabilityZoomRangeMillis
        ? Math.max(
            this.fromMillis,
            this.toMillis - this.chartOptions.defaultAvailabilityZoomRangeMillis
          )
        : this.fromMillis;
      this.sparkLineChart.setBrushStartTime(new Date(this.zoomedFromMillis));
      this.sparkLineChart.setBrushEndTime(new Date(this.zoomedToMillis));
      this.setFromAndToTimes(
        Math.max(this.fromMillis, this.toMillis - 24 * 60 * 60 * 1000),
        this.toMillis
      );
      this.setBrush(
        Math.max(this.fromMillis, this.toMillis - 24 * 60 * 60 * 1000),
  ...
  private buildZoomButtons() {
    ...
    let midpoint =
      (this.sparkLineChart.x.range()[1] - this.sparkLineChart.x.range()[0]) / 2;
    var buttonsDiv = this.targetElement
      .append('div')
  ...
  private renderDateTimeButton() {
    let minMillis =
      this.fromMillis +
      Utils.getOffsetMinutes(this.chartOptions.offset, this.fromMillis) * 60 * 1000;
    let maxMillis =
      this.toMillis +
      Utils.getOffsetMinutes(this.chartOptions.offset, this.toMillis) * 60 * 1000;
    let startMillis =
      this.selectedFromMillis +
      Utils.getOffsetMinutes(this.chartOptions.offset, this.selectedFromMillis) *
        60 *
        1000;
    let endMillis =
      this.selectedToMillis +
      Utils.getOffsetMinutes(this.chartOptions.offset, this.selectedFromMillis) *
        60 *
        1000;
    this.dateTimeButton.render(
      this.chartOptions,
      this.fromMillis,
      this.toMillis,
      this.selectedFromMillis,
      ...
      (fromMillis, toMillis, offset) => {
        ...
        (<any>(
          d3.select(this.renderTarget).select('.tsi-dateTimeContainer').node()
        )).focus();
      },
      () => {
        this.dateTimePickerContainer.style('display', 'none');
  ...
  private drawGhost() {
    var svgGroup = this.targetElement
      .select('.tsi-sparklineContainer')
      .select('.tsi-lineChartSVG')
      .select('.svgGroup');
    ...
    svgGroup
      ...
      .attr(
        'x',
        Math.max(
          this.sparkLineChart.x.range()[0],
          this.sparkLineChart.x(new Date(this.selectedFromMillis))
        )
      )
      .attr('y', 0)
      .attr(
        ...
        Math.min(
          Math.max(
            this.minGhostWidth,
            this.sparkLineChart.x(new Date(this.selectedToMillis)) -
              this.sparkLineChart.x(new Date(this.selectedFromMillis))
          ),
          this.sparkLineChart.x.range()[1] - this.sparkLineChart.x.range()[0]
        )
  ...
  private drawWarmRange() {
    ...
    if (svgGroup.select('.tsi-warmRect').empty()) {
      svgGroup.append('rect').classed('tsi-warmRect', true);
    ...
    if (this.chartOptions.warmStoreRange) {
      ...
      let boundedWarmStart = new Date(
        Math.max(warmStart.valueOf(), this.zoomedFromMillis)
      );
      let warmEnd = new Date(
        this.chartOptions.warmStoreRange.length === 2
          ? this.chartOptions.warmStoreRange[1]
          : this.toMillis
      );
      let boundedWarmEnd = new Date(
        Math.min(warmEnd.valueOf(), this.zoomedToMillis)
      );

      if (boundedWarmStart < boundedWarmEnd) {
        outOfRange = false;
        warmRect
          .attr('x', Math.max(this.timePickerLineChart.x(boundedWarmStart)))
          .attr('y', this.chartOptions.isCompact ? 12 : -8)
          .attr(
            'width',
            this.timePickerLineChart.x(boundedWarmEnd) -
              this.timePickerLineChart.x(boundedWarmStart)
          )
          .attr(
            'height',
            this.chartOptions.isCompact
              ? 4
              : Math.max(
                  this.targetElement
                    .select('.tsi-timePickerContainer')
          ...
          .attr('fill-opacity', this.chartOptions.isCompact ? 0.8 : 0.08)
          .attr('stroke-opacity', this.chartOptions.isCompact ? 0 : 0.5)
          .attr('pointer-events', 'none');
      }
    }
  ...
  private buildCompactFromAndTo() {
    ...
    if (this.chartOptions.persistDateTimeButtonRange) return;
    ...
    if (this.selectedFromMillis != null && this.selectedToMillis != null) {
      leftTimeText = this.timePickerContainer
        .append('div')
        .classed('tsi-compactFromTo', true)
        .style(
          'left',
          (brushPositions.leftPos != null
            ? Math.max(brushPositions.leftPos, 5)
            : 5) + 'px'
        )
        .text(
          Utils.timeFormat(
          ...
          )(new Date(this.selectedFromMillis))
        );
      let timezoneAbbreviation =
        ' (' + Utils.createTimezoneAbbreviation(this.chartOptions.offset) + ')';
      rightTimeText = this.timePickerContainer
        .append('div')
        .attr('class', 'tsi-compactFromTo')
        .style(
          'right',
          brushPositions.rightPos != null
            ? 'calc(100% - ' + brushPositions.rightPos + 'px)'
            : '5px'
        )
        .style('left', 'auto')
        .text(
    ...
    if (leftTimeText && rightTimeText) {
      var rightSideOfLeft =
        leftTimeText.node().getBoundingClientRect().left +
        leftTimeText.node().getBoundingClientRect().width;
      ...
      var minOffset = 40;
      if (leftSideOfRight - rightSideOfLeft < minOffset) {
        // if there is overlap (or near overlap), correction needed
        ...
        if (brushPositions.leftPos - correction < padding) {
          leftWeight =
            1 - (padding - (brushPositions.leftPos - correction)) / correction;
          rightWeight = 2 - leftWeight;
        }
        if (brushPositions.rightPos + correction > totalWidth - padding) {
          rightWeight =
            1 -
            (padding - (totalWidth - brushPositions.rightPos - correction)) /
              correction;
        ...
        rightTimeText
          .style(
            'right',
            'calc(100% - ' +
              Math.round(brushPositions.rightPos + rightWeight * correction) +
              'px)'
          )
          .style('left', 'auto');
        leftTimeText.style(
          'left',
          brushPositions.leftPos - leftWeight * correction + 'px'
        );
      }
    }
  }
  ...
  private buildFromAndToContainer() {
    let dateTimeContainer = this.timePickerTextContainer
      .append('div')
      .classed('tsi-dateTimeContainer', true);
    let timeframeLabel = dateTimeContainer
      .append('label')
      .text(this.getString('Timeframe'))
      .attr('id', 'time-frame-label');
    let dateTimeButtonContainer = dateTimeContainer
      .append('div')
  ...
  private setTicks() {
    ...
    let forceFirst =
      this.timePickerLineChart.zoomedFromMillis ==
        this.timePickerLineChart.fromMillis &&
      this.zoomedFromMillis == this.fromMillis;
    let forceLast =
      this.timePickerLineChart.zoomedToMillis == this.timePickerLineChart.toMillis &&
      this.zoomedToMillis == this.toMillis;
    this.timePickerLineChart.updateXAxis(forceFirst, forceLast);

    let ticks = this.timePickerContainer
      .select('.tsi-timePickerChart')
      .select('.xAxis')
      .selectAll('.tick');
  ...
  private setAvailabilityRange(fromMillis, toMillis) {
    ...
    this.timePickerLineChart.render(visibileAvailability, this.chartOptions, [
      aeWithNewTimeSpan,
    ]);
    this.setTicks();
    this.drawWarmRange();
