...
class StateSeries extends TimelineComponent { 
	...
	public render(namedData: Array<any>, options: any = {}){
		...
		var timeFormat = (d, i) => { 
			...
			return Utils.timeFormat(this.usesSeconds, this.usesMillis, this.chartOptions.offsetMinutes)(startTime) + " - " + 
				   Utils.timeFormat(this.usesSeconds, this.usesMillis, this.chartOptions.offsetMinutes)(endTime);
		}

		var self = this;
		enteredRects.on("mousemove", function (dRect, iRect) { 
			self.elementMouseover(dRect, iRect, (d, i) => { 
				return Utils.timeFormat(this.usesSeconds, this.usesMillis, this.chartOptions.offsetMinutes)(new Date(d.time));
			});
		...
		if (this.chartOptions.xAxisHidden != true) {
			...
			this.xAxis.transition()
				.duration(this.TRANSDURATION)
				.call(d3.axisBottom(this.xScale).tickFormat(Utils.timeFormat(this.usesSeconds, this.usesMillis, this.chartOptions.offsetMinutes)))
				.tween("", () => {
					return () => {
