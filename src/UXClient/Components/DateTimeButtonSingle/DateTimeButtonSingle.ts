...
class DateTimeButtonSingle extends DateTimeButton {    
    ...
    public render (chartOptions: any = {}, minMillis: number, maxMillis: number, selectedMillis: number = null, onSet = null) {
        ...
        d3.select("html").on("click." + Utils.guid(), (event) => {
            ...
            var outside = dateTimeTextChildren.filter(() => { return this === event.target; }).empty() 
                && targetElement.selectAll(".tsi-dateTimePickerContainer").filter(() => { return this === event.target; }).empty()
                && targetElement.selectAll(".tsi-dateTimeButton").filter(() => { return this === event.target; }).empty()
                && targetElement.selectAll(".tsi-saveButton").filter(() => { return this === event.target; }).empty();
            var inClickTarget = pickerContainerChildren.filter(() => { return this === event.target; }).empty();
