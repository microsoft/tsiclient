...
import {
  DataTypes,
  YAxisStates,
  TooltipMeasureFormat,
} from './../../Constants/Enums';
import {
  LINECHARTTOPPADDING,
  LINECHARTCHARTMARGINS,
  VALUEBARHEIGHT,
  SERIESLABELWIDTH,
...
class LineChart extends TemporalXAxisComponent {
  ...
  private nonFocusStrokeOpactiy = 0.3;
  ...
  private horizontalLabelOffset =
    LINECHARTCHARTMARGINS.left + swimlaneLabelConstants.leftMarginOffset;
  ...
  private resetValueElementsFocus = () => {
    ...
    this.svgSelection.selectAll('.tsi-valueEnvelope').attr('fill-opacity', 0.2);

    Utils.revertAllSubtitleText(
  ...
  private voronoiMouseout(event, d: any) {
    ...
    if (this.contextMenu && this.contextMenu.contextMenuVisible) return;

    this.hideFocusElements();
    this.tooltip.hide();
    (<any>this.legendObject.legendElement.selectAll('.tsi-splitByLabel')).classed(
      'inFocus',
      false
    );
  ...
  private createMarkerInstructions() {
    ...
    this.targetElement
      ...
      .text(
        this.getString('Click to drop marker') +
          ',' +
          this.getString('drag to reposition') +
          '.'
      );
  }
  ...
  private focusOnlyHoveredSeries = (
    aggKey,
    splitBy,
    shouldSetFocusedValues: boolean
  ) => {
    if (aggKey !== this.focusedAggKey || splitBy !== this.focusedSplitby) {
      ...
      this.svgSelection.selectAll('.tsi-valueEnvelope').attr('fill-opacity', 0.1);
  ...
  private setHorizontalValuePosAndText(
    ...
    this.horizontalValueBox
      ...
      .text(
        Utils.timeFormat(
          this.chartComponentData.usesSeconds,
          this.chartComponentData.usesMillis,
          this.chartOptions.offset,
          this.chartOptions.is24HourTime,
          shiftMillis,
          null,
  ...
  private voronoiMouseover = (d: any) => {
    ...
    this.setHorizontalValuePosAndText(
      d,
      xPos + this.getSVGLeftOffset() + this.chartMargins.left,
      xValue,
      shiftMillis
    );
    ...
    if (this.chartOptions.tooltip) {
      ...
      this.tooltip.draw(
        ...
        20,
        20,
        this.colorMap[d.aggregateKey + '_' + d.splitBy]
      );
    } else this.tooltip.hide();
  ...
  private getYExtent(aggValues, isEnvelope, aggKey = null) {
    ...
    if (
      aggKey !== null &&
      this.chartComponentData.displayState[aggKey].yExtent !== null
    ) {
    ...
    if (isEnvelope) {
      ...
      filteredValues.forEach((d: any) => {
        if (this.chartComponentData.isPossibleEnvelope(d.aggregateKey, d.splitBy)) {
          ...
          var visibleMeasure = this.chartComponentData.getVisibleMeasure(
            d.aggregateKey,
            d.splitBy
          );
          if (
            d.measures[visibleMeasure] != undefined &&
            d.measures[visibleMeasure] != null
          ) {
  ...
  private getFilteredAndSticky(aggValues) {
    //getFilteredValues, then filter by sticky
    var filteredValues = this.getFilteredValues(aggValues);
    let numericFiltered = filteredValues.filter((d: any) => {
      return this.getDataType(d.aggregateKey) === DataTypes.Numeric;
    });
    if (this.chartComponentData.stickiedKey == null) return numericFiltered;
    return numericFiltered.filter((d: any) => {
      return (
        d.aggregateKey == this.chartComponentData.stickiedKey.aggregateKey &&
        (this.chartComponentData.stickiedKey.splitBy == null
          ? true
  ...
  public stickyOrUnstickySeries = (aggKey, splitBy) => {
    if (
      ...
      this.chartComponentData.stickiedKey.aggregateKey === aggKey &&
  ...
  public unstickySeries = (aggKey, splitby = null) => {
    if (
      this.getDataType(aggKey) !== DataTypes.Numeric ||
      !this.chartOptions.shouldSticky
    ) {
      return;
    }
    ...
    this.voronoiDiagram = this.voronoi(
      this.getFilteredAndSticky(this.chartComponentData.allValues)
    );
  ...
  private stickySeries = (aggregateKey: string, splitBy: string = null) => {
    if (
      this.getDataType(aggregateKey) !== DataTypes.Numeric ||
      !this.chartOptions.shouldSticky
    ) {
      return;
    }
    var filteredValues = this.getFilteredAndSticky(
      this.chartComponentData.allValues
    );
    if (filteredValues == null || filteredValues.length == 0) return;
    this.focusedAggKey = null;
    this.focusedSplitby = null;
    ...
    (<any>(
      this.legendObject.legendElement
        .selectAll('.tsi-splitByLabel')
        .filter(function (filteredSplitBy: any) {
          return (
            d3.select(this.parentNode).datum() == aggregateKey &&
            filteredSplitBy == splitBy
          );
        })
  ...
  public setBrush() {
    if (this.brushStartTime && this.brushEndTime && this.brushElem && this.brush) {
      ...
      if (
        rawRightSide < this.xOffset ||
        rawLeftSide > this.chartWidth - 2 * this.xOffset
      ) {
      ...
      let leftSide = Math.min(
        this.chartWidth - 2 * this.xOffset,
        Math.max(this.xOffset, this.x(this.brushStartTime))
      );
      let rightSide = Math.min(
        this.chartWidth - 2 * this.xOffset,
        Math.max(this.xOffset, this.x(this.brushEndTime))
      );
  ...
  private findClosestValidTime(rawMillis: number) {
    ...
    return Object.keys(this.chartComponentData.timeMap).reduce(
      (closestValue: number, currValue: any) => {
  ...
  private getMarkerMarginLeft() {
    var legendWidth = this.legendObject.legendElement
      .node()
      .getBoundingClientRect().width;
    return (
      this.chartMargins.left +
      (this.chartOptions.legend === 'shown' || this.chartOptions.legend === 'hidden'
        ? legendWidth
        : 0) +
  ...
  public exportMarkers() {
    this.chartOptions.markers = Object.keys(this.markerGuidMap)
      .filter(
        (markerGuid) =>
          !this.activeMarker || this.activeMarker.getGuid() !== markerGuid
      )
      .map((markerGuid) => {
        return [
          this.markerGuidMap[markerGuid].getMillis(),
          this.markerGuidMap[markerGuid].getLabelText(),
        ];
      });
  ...
  private createOnMarkerChange(markerGuid: string, marker: any) {
    return (isDeleting, droppedMarker, shouldSort: boolean = true) => {
      if (droppedMarker) {
        ...
        let visibleMarkers: any = Object.values(this.markerGuidMap).filter(
          (marker: Marker) => {
      ...
      this.exportMarkers();
      if (shouldSort) this.sortMarkers();
    };
  }
  ...
  private renderMarker(
    ...
    isSeriesLabels: boolean = false
  ) {
    marker.render(millis, this.chartOptions, this.chartComponentData, {
      chartMargins: this.chartMargins,
      x: this.x,
      marginLeft:
        this.getMarkerMarginLeft() +
        (isSeriesLabels ? this.getAdditionalOffsetFromHorizontalMargin() : 0),
  ...
  public addMarker = () => {
    ...
    this.renderMarker(
      ...
      Infinity,
      onChange,
      `${this.getString('Marker')} ${Object.keys(this.markerGuidMap).length}`
    );
  };
  ...
  private voronoiMousemove(mouseEvent) {
    ...
    if (!this.isDroppingMarker) {
      ...
      let closestTime = Utils.findClosestTime(
        rawTime.valueOf(),
        this.chartComponentData.timeMap
      );
    ...
    if (
      site.data.aggregateKey !== this.focusedAggKey ||
      site.data.splitBy !== this.focusedSplitby
    ) {
      let selectedFilter = Utils.createValueFilter(
        site.data.aggregateKey,
        site.data.splitBy
      );
      this.focusMarkerLabel(
        selectedFilter,
        site.data.aggregateKey,
        site.data.splitBy
      );
  ...
  private voronoiContextMenu(d3Event, mouseEvent) {
    ...
    if (
      this.chartComponentData.displayState[site.data.aggregateKey]
        .contextMenuActions &&
      this.chartComponentData.displayState[site.data.aggregateKey].contextMenuActions
        .length
    ) {
      ...
      let eventSite = {
        pageX: sitePageCoords.left + site[0],
        pageY: sitePageCoords.top + site[1] - 12,
      };
    ...
    if (!this.isDroppingMarker) {
      if (site.data && cDO.onElementClick !== null) {
        cDO.onElementClick(
          site.data.aggregateKey,
          site.data.splitBy,
          site.data.dateTime.toISOString(),
          site.data.measures
        );
      } else {
  ...
  private getValueOfVisible(d) {
    return Utils.getValueOfVisible(
      d,
      this.chartComponentData.getVisibleMeasure(d.aggregateKey, d.splitBy)
    );
  }
  ...
  private brushBrush(event) {
    ...
    if (this.brushContextMenu) this.brushContextMenu.hide();
  ...
  private brushEnd(d3Event, mouseEvent) {
    ...
    if (
      d3Event &&
      d3Event.selection == null &&
      d3Event.sourceEvent &&
      d3Event.sourceEvent.type == 'mouseup' &&
      this.chartOptions.minBrushWidth == 0
    ) {
      if (this.brushContextMenu) {
        this.brushContextMenu.hide();
      }
      ...
      var site: any = this.voronoiDiagram.find(mx, my);
      let isClearingBrush =
        this.brushStartPosition !== null && this.brushEndPosition !== null;
      if (
        this.chartComponentData.stickiedKey !== null &&
        !this.isDroppingMarker &&
        !isClearingBrush
      ) {
        this.chartComponentData.stickiedKey = null;
        (<any>(
          this.legendObject.legendElement.selectAll('.tsi-splitByLabel')
        )).classed('stickied', false);
        // recompute voronoi with no sticky
        this.voronoiDiagram = this.voronoi(
          this.getFilteredAndSticky(this.chartComponentData.allValues)
        );
      ...
      if (
        !this.isDroppingMarker &&
        !isClearingBrush &&
        !(this.contextMenu && this.contextMenu.contextMenuVisible)
      ) {
        this.stickySeries(site.data.aggregateKey, site.data.splitBy);
      } else {
        this.isDroppingMarker = false;
    ...
    if (d3Event.selection == null) {
      if (!this.chartOptions.brushClearable) {
        d3.select(mouseEvent)
          .transition()
          .call(d3Event.target.move, [
            this.x(this.brushStartTime),
            this.x(this.brushEndTime),
          ]);
      }
      return;
    }
    ...
    if (this.chartOptions.snapBrush) {
      ...
      if (this.possibleTimesArray.length > 0) {
        ...
        if (
          newBrushStartTime != this.brushStartTime ||
          newBrushEndTime != this.brushEndTime
        ) {
    ...
    if (d3Event.selection[1] - d3Event.selection[0] < this.minBrushWidth) {
      let rightSide = Math.min(
        d3Event.selection[0] + this.minBrushWidth,
        this.x.range()[1]
      );
      transformCall = () =>
        d3
          .select(mouseEvent)
          .transition()
          .call(d3Event.target.move, [rightSide - this.minBrushWidth, rightSide]);
      isZeroWidth = rightSide - this.minBrushWidth == rightSide;
    }
    if (
      this.chartOptions.brushMoveEndAction &&
      d3Event.sourceEvent &&
      d3Event.sourceEvent.type == 'mouseup'
    ) {
    ...
    if (
      this.chartOptions.brushContextMenuActions &&
      this.chartOptions.autoTriggerBrushContextMenu &&
      d3Event.sourceEvent &&
      d3Event.sourceEvent.type == 'mouseup'
    ) {
      if (
        !this.chartOptions.brushContextMenuActions ||
        this.chartOptions.brushContextMenuActions.length == 0
      )
        return;
      var mousePosition = d3.pointer(d3Event, <any>this.targetElement.node());
      //constrain the mouse position to the renderTarget
      var boundingCRect = this.targetElement.node().getBoundingClientRect();
      var correctedMousePositionX = Math.min(
        boundingCRect.width,
        Math.max(mousePosition[0], 0)
      );
      var correctedMousePositionY = Math.min(
        boundingCRect.height,
        Math.max(mousePosition[1], 0)
      );
      var correctedMousePosition = [
        correctedMousePositionX,
        correctedMousePositionY,
      ];

      this.brushContextMenu.draw(
        this.chartComponentData,
        this.renderTarget,
        this.chartOptions,
        correctedMousePosition,
        null,
        null,
        null,
        this.brushStartTime,
  ...
  private focusMarkerLabel(filterFunction, aggKey, splitBy) {
    ...
    d3.select(this.renderTarget)
      .selectAll('.tsi-markerValue')
      .filter(filterFunction)
      .style('opacity', 1)
      .classed('tsi-isExpanded', true)
      .each(function () {
        Utils.setSeriesLabelSubtitleText(
          d3.select(this).selectAll('.tsi-tooltipSubtitle'),
          true
        );
      });
      ...
      .each(function () {
        d3.select(this)
          ...
          .sort(function (a: any, b: any) {
            return a.aggregateKey == aggKey &&
              (splitBy == null || splitBy == a.splitBy)
              ? 1
              : -1;
          });
      });
  ...
  public labelMouseout = () => {
    if (this.svgSelection) {
      ...
      this.svgSelection
        ...
        .attr('fill-opacity', 1);
      this.svgSelection.selectAll('.tsi-valueEnvelope').attr('fill-opacity', 0.3);
    }
  };
  ...
  private drawBrushRange() {
    if (this.chartOptions.brushRangeVisible) {
      if (
        this.targetElement.select('.tsi-rangeTextContainer').empty() &&
        (this.brushStartTime || this.brushEndTime)
      ) {
        var rangeTextContainer = this.targetElement
          .append('div')
  ...
  private getSVGLeftOffset() {
    return this.chartOptions.legend === 'shown'
      ? this.width - this.svgSelection.node().getBoundingClientRect().width
      : 0;
  }
  ...
  public updateBrushRange() {
    ...
    let rangeText = Utils.rangeTimeFormat(
      this.brushEndTime.valueOf() - this.brushStartTime.valueOf()
    );
    let rangeTextContainer = this.targetElement.select('.tsi-rangeTextContainer');

    let leftPos =
      this.chartMargins.left +
      Math.min(Math.max(0, this.x(this.brushStartTime)), this.x.range()[1]) +
      svgLeftOffset;

    let rightPos =
      this.chartMargins.left +
      Math.min(Math.max(0, this.x(this.brushEndTime)), this.x.range()[1]) +
      svgLeftOffset;
  ...
  private nextStackedState = () => {
    if (this.chartOptions.yAxisState === YAxisStates.Stacked) return 'shared';
    else if (this.chartOptions.yAxisState === YAxisStates.Shared) return 'overlap';
    else return 'stacked';
  };
  ...
  private getVisibleNumerics() {
    let visibleGroups = this.data.filter(
      (agg) => this.chartComponentData.displayState[agg.aggKey]['visible']
    );
    let visibleCDOs = this.aggregateExpressionOptions.filter(
      (cDO) => this.chartComponentData.displayState[cDO.aggKey]['visible']
    );
  ...
  private getSwimlaneOffsets(
    linechartTopPadding: number,
    visibleGroups: Array<ChartDataOptions>,
    visibleCDOs: Array<ChartDataOptions>,
    heightPerNumeric: number,
    swimLaneSet: any
  ) {
    ...
    Object.keys(swimLaneSet)
      ...
      .forEach((swimLaneStr) => {
        ...
        visibleCDOs.forEach((aggGroup, i) => {
          if (
            aggGroup.swimLane === swimlane &&
            aggGroup.dataType === DataTypes.Numeric
          ) {
            hasNumeric = true;
        ...
        if (
          ...
          this.chartOptions.swimLaneOptions[swimlane] &&
          this.chartOptions.swimLaneOptions[swimlane].collapseEvents
        ) {
          swimLaneOffset += this.getEventsCollapsedSwimlaneHeight(
            visibleCDOs,
            swimlane
          );
          visibleCDOs.forEach((aggGroup, i) => {
            if (aggGroup.swimLane === swimlane) {
              visibleGroupEndValues[i] = [
                cumulativeOffset,
                this.getEventsCollapsedSwimlaneHeight(visibleCDOs, swimlane),
              ];
            }
          });
        } else {
          visibleCDOs.forEach((aggGroup, i) => {
            if (
              ...
              visibleGroupEndValues[i] = [
                swimLaneOffset + cumulativeOffset,
                currGroupsHeight,
              ];
  ...
  private setSwimLaneYExtents(
    ...
    swimLanes.forEach((lane) => {
      ...
      if (
        swimLaneOptions &&
        swimLaneOptions[lane] &&
        swimLaneOptions[lane].yExtent
      ) {
      ...
      visibleGroups.forEach((aggGroup, i) => {
        ...
        if (cDO.swimLane === lane) {
          ...
          Object.keys(this.chartComponentData.visibleTAs[aggKey]).forEach(
            (splitBy) => {
              aggValues = aggValues.concat(
                this.chartComponentData.visibleTAs[aggKey][splitBy]
              );
            }
          );
          let yExtent = this.getYExtent(
            aggValues,
            this.chartComponentData.displayState[aggKey].includeEnvelope
              ? this.chartComponentData.displayState[aggKey].includeEnvelope
              : this.chartOptions.includeEnvelope,
            aggKey
          );
  ...
  private getEventsCollapsedSwimlaneHeight(visibleCDOs, swimlane) {
    ...
    let rawHeight = visibleCDOs.reduce((tallest, currGroup) => {
      if (
        currGroup.dataType === DataTypes.Events &&
        currGroup.swimLane === swimlane
      ) {
  ...
  private createYOffsets() {
    ...
    visibleCDOs.forEach((cDO) => {
      swimLaneSet[cDO.swimLane] =
        swimLaneSet[cDO.swimLane] || cDO.dataType === DataTypes.Numeric;
    });
    visibleNumericCount = Object.keys(swimLaneSet).reduce(
      (visibleCount, swimLane) => {
    ...
    if (this.chartOptions.swimLaneOptions) {
      Object.keys(this.chartOptions.swimLaneOptions).forEach((swimlaneKey) => {
        ...
        if (sLO.collapseEvents) {
          let swimlaneHeight = this.getEventsCollapsedSwimlaneHeight(
            visibleCDOs,
            swimlane
          );
    ...
    let heightNonNumeric = visibleCDOs.reduce((sumPrevious, currGroup, i) => {
      if (
        currGroup.dataType === DataTypes.Events &&
        this.chartOptions.swimLaneOptions &&
        this.chartOptions.swimLaneOptions[currGroup.swimLane] &&
        this.chartOptions.swimLaneOptions[currGroup.swimLane].collapseEvents
      ) {
        return sumPrevious;
      }
      return (
        sumPrevious +
        (currGroup.dataType !== DataTypes.Numeric
          ? Utils.getNonNumericHeight(currGroup.height)
          : 0)
      );
    }, 0);
    ...
    return this.getSwimlaneOffsets(
      linechartTopPadding,
      visibleGroups,
      visibleCDOs,
      heightPerNumeric,
  ...
  private getGroupYExtent(aggKey, aggVisible, aggValues, yExtent) {
    if (
      this.chartOptions.yAxisState === YAxisStates.Shared ||
      Object.keys(this.chartComponentData.timeArrays).length < 2 ||
      !aggVisible
    ) {
      yExtent = this.getYExtent(
        this.chartComponentData.allNumericValues,
        this.chartComponentData.displayState[aggKey].includeEnvelope
          ? this.chartComponentData.displayState[aggKey].includeEnvelope
          : this.chartOptions.includeEnvelope,
        null
  ...
  private getAggAxisType(agg) {
    if (this.chartOptions.yAxisState === YAxisStates.Stacked) {
      if (
        this.chartOptions.swimLaneOptions &&
        this.chartOptions.swimLaneOptions[agg.swimLane] &&
        this.chartOptions.swimLaneOptions[agg.swimLane].yAxisType
      ) {
  ...
  private adjustSwimLanes() {
    if (
      this.chartOptions.yAxisState === YAxisStates.Shared ||
      this.chartOptions.yAxisState === YAxisStates.Overlap
    ) {
      ...
      if (this.chartOptions.swimLaneOptions) {
        ...
        this.chartOptions.swimLaneOptions = {
          0: {
            yAxisType: this.chartOptions.yAxisState,
            horizontalMarkers: horizontalMarkers,
          },
        };
      }
    } else {
      let minimumPresentSwimLane = this.aggregateExpressionOptions.reduce(
        (currMin, aEO) => {
  ...
  private getHorizontalMarkersWithYScales() {
    ...
    const pushMarker = (cDO, marker, markerList) => {
      ...
      const domain =
        this.chartOptions.yAxisState === YAxisStates.Stacked
      ...
      if (
        domain &&
        this.chartOptions.swimLaneOptions?.[cDO.swimLane]?.yAxisType !==
          YAxisStates.Overlap &&
    ...
    if (
      ...
      Object.keys(this.chartOptions.swimLaneOptions).forEach((swimlaneNumber) => {
        ...
        swimlaneOptions.horizontalMarkers?.forEach(
          (horizontalMarkerParams: HorizontalMarker) => {
  ...
  private drawHorizontalMarkers() {
    ...
    const markerContainers = this.svgSelection
      .select('.svgGroup')
      .selectAll('.tsi-horizontalMarker')
      .data(markerList);
    markerContainers
      .enter()
      .append('g')
  ...
  private createSwimlaneLabels(offsetsAndHeights, visibleCDOs) {
    ...
    const useAggForLaneLabel = (aggGroup) => {
      ...
      let onClick = null;
      if (
        typeof this.chartOptions?.swimLaneOptions?.[swimLane]?.onClick === 'function'
      ) {
        onClick = () =>
          this.chartOptions?.swimLaneOptions?.[swimLane]?.onClick?.(swimLane);
      }
    ...
    visibleCDOs
      .filter((aggGroup) => aggGroup.dataType === DataTypes.Numeric)
      .forEach((aggGroup) => {
        if (!(aggGroup.swimLane in swimLaneLabels)) {
          // Only add swimlanes once to swimLaneLabels map
    ...
    visibleCDOs
      ...
      .forEach((aggGroup) => {
        ...
        if (!(aggGroup.swimLane in swimLaneLabels)) {
          ...
          // if lane contains non-numeric data and is being added to another lane
          if (
            !this.chartOptions?.swimLaneOptions?.[aggGroup.swimLane]?.collapseEvents
          ) {
            // Only increment event heights if collapseEvents === false
            swimLaneLabels[aggGroup.swimLane].height +=
              offsetsAndHeights[aggIndex][1]; // add heights (non-numerics don't share Y axis)
          }
        }
      });
    ...
    const truncateLabel = (labelRef: HTMLElement, data) => {
      const maxHeight =
        data.height - swimlaneLabelConstants.swimLaneLabelHeightPadding; // padding on actual lane height
      if (data.label) {
    ...
    const onClickPresentAndValid = (dp) =>
      dp.onClick && typeof dp.onClick === 'function';

    // Map over swimLanes and create labels
    Object.keys(swimLaneLabels).forEach((lane) => {
      let labelData = [swimLaneLabels[lane]];
      let label = this.swimLaneLabelGroup
        .selectAll(`tsi-swimLaneLabel-${lane}`)
        .data(labelData);
      ...
      label
        ...
        .attr(
          'class',
          (d) =>
            `tsi-swimLaneLabel-${lane} tsi-swimLaneLabel ${
              onClickPresentAndValid(d) ? 'tsi-boldOnHover' : ''
            }`
        )
        .merge(label)
        ...
        .attr(
          ...
          (d) =>
            `translate(${
              -this.horizontalLabelOffset + swimlaneLabelConstants.labelLeftPadding
            },${d.offset + d.height / 2}) rotate(-90)`
        )
        .text((d) => d.label)
        .each(function (d) {
          truncateLabel(this, d);
        })
        .on('click', (event, d) => {
  ...
  public render(data: any, options: any, aggregateExpressionOptions: any) {
    ...
    this.hasBrush =
      options &&
      (options.brushMoveAction ||
        options.brushMoveEndAction ||
        options.brushContextMenuActions);
    this.chartOptions.setOptions(options);
    this.chartMargins.right = this.chartOptions.labelSeriesWithMarker
      ? SERIESLABELWIDTH + 8
      : LINECHARTCHARTMARGINS.right;
    this.width = this.getWidth();
    this.height = Math.max(
      (<any>d3.select(this.renderTarget).node()).clientHeight,
      this.MINHEIGHT
    );
    if (this.chartOptions.legend == 'compact') this.chartMargins.top = 72;
    else this.chartMargins.top = 40;
    ...
    this.strokeOpacity = this.chartOptions.isArea ? 0.55 : 1;
    this.nonFocusStrokeOpactiy = this.chartOptions.isArea ? 0.55 : 0.3;

    this.chartComponentData.mergeDataToDisplayStateAndTimeArrays(
      this.data,
      this.aggregateExpressionOptions
    );
    ...
    this.chartHeight = Math.max(
      1,
      this.height - this.chartMargins.bottom - this.chartMargins.top
    );
    this.chartWidth = this.getChartWidth();

    if (
      this.brush &&
      this.svgSelection.select('.svgGroup').select('.brushElem') &&
      !this.chartOptions.keepBrush
    ) {
      this.brushStartTime = null;
      this.brushEndTime = null;
      this.brushStartPosition = null;
    ...
    if (
      !this.chartOptions.hideChartControlPanel &&
      this.chartControlsPanel === null
    ) {
      this.chartControlsPanel = Utils.createControlPanel(
        this.renderTarget,
        this.legendWidth + this.GUTTERWIDTH / 2,
        Math.max(this.chartMargins.top + 12, 0),
        this.chartOptions
      );
      var self = this;
      this.hasStackedButton = true;
      this.stackedButton = this.chartControlsPanel
        .append('button')
        .style('left', '60px')
        .attr('class', 'tsi-stackedButton')
        .attr(
          'aria-label',
          () => this.getString('set axis state to') + ' ' + this.nextStackedState()
        )
        ...
        .on('click', function () {
          ...
          self.render(
            ...
            self.aggregateExpressionOptions
          );
          d3.select(this).attr(
            'aria-label',
            () => self.getString('set axis state to') + ' ' + self.nextStackedState()
          );
    ...
    if (this.chartControlsPanel !== null) {
      ...
      this.chartControlsPanel.style(
        'top',
        Math.max(this.chartMargins.top - 24, 0) + 'px'
      );
    }
    ...
    if (this.svgSelection == null) {
      ...
      this.brushElem = null;
      if (this.hasBrush) {
        this.brushElem = g.append('g').attr('class', 'brushElem');
        this.brushElem.classed(
          'hideBrushHandles',
          !this.chartOptions.brushHandlesVisible
        );
      } else {
      ...
      this.verticalValueBox = d3
        .select(this.renderTarget)
        .append('div')
        .attr('class', 'tsi-verticalValueBox')
        .style('display', 'none');

      this.horizontalValueBar = this.focus
        .append('line')
        .attr('y1', 0)
        .attr('y2', 0)
      ...
      this.swimLaneLabelGroup = g.append('g').attr('class', 'tsi-swimLaneLabels');
      ...
      this.draw = (isFromResize = false, event) => {
        this.minBrushWidth = this.chartOptions.minBrushWidth
          ? this.chartOptions.minBrushWidth
          : this.minBrushWidth;
        this.focus.attr(
          'visibility',
          this.chartOptions.focusHidden ? 'hidden' : 'visible'
        );
        this.verticalValueBox.style(
          'visibility',
          this.chartOptions.focusHidden ? 'hidden' : 'visible'
        );
        this.horizontalValueBox.style(
          'visibility',
          this.chartOptions.focusHidden ? 'hidden' : 'visible'
        );
        if (this.chartOptions.xAxisHidden && this.chartOptions.focusHidden) {
        ...
        this.width = Math.max(
          (<any>d3.select(this.renderTarget).node()).clientWidth,
          this.MINWIDTH
        );
        if (!isFromResize) {
          this.chartWidth = this.getChartWidth();
        ...
        this.x = d3
          ...
          .rangeRound([
            this.xOffset,
            Math.max(this.xOffset, this.chartWidth - 2 * this.xOffset),
          ]);
        ...
        this.y = d3
          ...
          .range([
            Math.max(
              this.chartHeight - this.heightNonNumeric(),
              this.chartOptions.aggTopMargin
            ) - LINECHARTTOPPADDING,
            this.chartOptions.aggTopMargin,
          ]);
        ...
        var xExtent: any =
          this.chartComponentData.allValues.length != 0
            ? d3.extent(this.chartComponentData.allValues, (d: any) => d.dateTime)
            : [0, 1];
        var xRange =
          this.chartComponentData.allValues.length != 0
            ? Math.max(2, xExtent[1].valueOf() - xExtent[0].valueOf())
            : 2;
        var xOffsetPercentage = this.xOffset / this.chartWidth;
        if (this.chartOptions.timeFrame) {
          fromAndTo = [
            new Date(this.chartOptions.timeFrame[0]),
            new Date(this.chartOptions.timeFrame[1]),
          ];
        }
        ...
        var startOfBuckets = this.chartComponentData.allValues.map(
          (d: any) => d.dateTime
        );
        var endOfBuckets = this.chartComponentData.allValues
          .filter((d: any) => {
            return d.bucketSize != null;
          })
          .map((d: any) => {
            return new Date(d.dateTime.valueOf() + d.bucketSize);
          });
        var allPossibleTimes = startOfBuckets.concat(endOfBuckets);
        var timeSet = new Set(allPossibleTimes);
        this.possibleTimesArray = Array.from(timeSet.values())
          .sort()
          .map((ts: string) => {
            return new Date(ts);
          });
        ...
        if (this.voronoiRegion) {
          this.voronoiRegion
            .attr('x', xOffsetPercentage * this.chartWidth)
        ...
        if (this.brushElem) {
          ...
          this.brush = d3
            ...
            .extent([
              [
                this.xLowerBound,
                Math.min(this.chartHeight, this.chartOptions.aggTopMargin),
              ],
        ...
        var yExtent: any = this.getYExtent(
          this.chartComponentData.allValues,
          false,
          null
        );
        var yRange = yExtent[1] - yExtent[0] > 0 ? yExtent[1] - yExtent[0] : 1;
        var yOffsetPercentage = this.chartOptions.isArea
          ? 1.5 / this.chartHeight
          : 10 / this.chartHeight;
        this.y.domain([
          yExtent[0] - yRange * yOffsetPercentage,
          yExtent[1] + yRange * (10 / this.chartHeight),
        ]);
          ...
          this.areaPath = d3
            ...
            .defined((d: any) => {
              return (
                d.measures !== null &&
                d.measures[
                  this.chartComponentData.getVisibleMeasure(
                    d.aggregateKey,
                    d.splitBy
                  )
                ] !== null
        ...
        if (!this.chartOptions.xAxisHidden) {
          ...
          var xAxisBaselineEntered = xAxisBaseline
            .enter()
            .append('line')
            .attr('class', 'xAxisBaseline')
            .attr('x1', 0.5)
            .merge(xAxisBaseline)
            .attr('y2', this.chartHeight + 0.5)
            .attr('y1', this.chartHeight + 0.5)
            .attr('x2', this.chartWidth - this.xOffset);
          xAxisBaseline.exit().remove();
        }
        if (g.selectAll('.xAxis').size() !== 0) {
          g.selectAll('.xAxis').style(
            'visibility',
            !this.chartOptions.xAxisHidden ? 'visible' : 'hidden'
          );
        }

        /******************** Draw Line and Points ************************/
        this.visibleAggCount = Object.keys(
          this.chartComponentData.timeArrays
        ).reduce((count: number, aggKey: string): number => {
          return (
            count + (this.chartComponentData.displayState[aggKey]['visible'] ? 1 : 0)
          );
        }, 0);
        ...
        let visibleGroupData = this.chartComponentData.data.filter(
          (agg) => this.chartComponentData.displayState[agg.aggKey]['visible']
        );
        ...
        let aggregateGroups = this.svgSelection
          .select('.svgGroup')
          .selectAll('.tsi-aggGroup')
        ...
        aggregateGroups
          ...
          .attr('transform', (agg, i) => {
            return self.chartOptions.isArea
              ? null
              : 'translate(0,' + offsetsAndHeights[i][0] + ')';
          })
          .each(function (agg, i) {
            let yExtent;
            let aggVisible = true;
            ...
            Object.keys(self.chartComponentData.visibleTAs[aggKey]).forEach(
              (splitBy) => {
                aggValues = aggValues.concat(
                  self.chartComponentData.visibleTAs[aggKey][splitBy]
                );
              }
            );
            ...
            if (
              self.plotComponents[aggKey] === undefined ||
              self.mismatchingChartType(aggKey)
            ) {
            ...
            let mouseoverFunction = self.getMouseoverFunction(
              visibleCDOs[i].dataType
            );
            ...
            if (agg.dataType === 'numeric') {
              let idx = self.aggregateExpressionOptions.findIndex(
                (el) => el.aggKey === aggKey
              );
            ...
            let axisState = new AxisState(
              self.getAggAxisType(agg),
              yExtent,
              positionInGroup
            );
            ...
            if (
              typeof self.chartOptions?.swimLaneOptions?.[swimLane]?.onClick ===
              'function'
            ) {
              yAxisOnClick = () =>
                self.chartOptions.swimLaneOptions[swimLane].onClick?.(swimLane);
            }

            self.plotComponents[aggKey].render(
              self.chartOptions,
              visibleNumericI,
              agg,
              true,
              d3.select(this),
              self.chartComponentData,
              axisState,
              self.chartHeight,
              self.visibleAggCount,
              self.colorMap,
        ...
        this.voronoi = d3Voronoi
          .voronoi()
          .x(function (d: any) {
            return d.bucketSize != undefined
              ? self.x(new Date(d.dateTime.valueOf() + d.bucketSize / 2))
              : self.x(d.dateTime);
          })
          .y(function (d: any) {
            if (d.measures) {
              return self.yMap[d.aggregateKey]
                ? self.yMap[d.aggregateKey](self.getValueOfVisible(d))
                : null;
            }
            return null;
          })
        ...
        var voronoiSelection = this.brushElem
          ? this.brushElem.select('.overlay')
          : this.voronoiRegion;
        ...
        if (this.brushElem) {
          this.brushElem
            ...
            .on('contextmenu', function (event, d) {
              if (
                !self.chartOptions.brushContextMenuActions ||
                self.chartOptions.brushContextMenuActions.length == 0 ||
                self.chartOptions.autoTriggerBrushContextMenu
              )
                return;
              var mousePosition = d3.pointer(event, <any>self.targetElement.node());
              event.preventDefault();
              self.brushContextMenu.draw(
                self.chartComponentData,
                self.renderTarget,
                self.chartOptions,
                mousePosition,
          ...
          this.brushElem
            ...
            .attr(
              'stroke',
              this.chartOptions.color ? this.chartOptions.color : 'none'
            )
            .attr(
              'fill',
              this.chartOptions.color ? this.chartOptions.color : 'grey'
            );
        ...
        if (this.hasStackedButton) {
          this.stackedButton
            .style('opacity', () => {
              ...
              if (this.chartOptions.yAxisState === YAxisStates.Shared) return 0.6;
              return 0.3;
            })
        ...
        let timeFrame = this.chartOptions.timeFrame
    ...
    if (
      this.chartOptions.labelSeriesWithMarker &&
      this.seriesLabelsMarker === null
    ) {
    ...
    d3.select('html').on('click.' + Utils.guid(), (event) => {
      if (
        this.ellipsisContainer &&
        event.target != this.ellipsisContainer.select('.tsi-ellipsisButton').node()
      ) {
        this.ellipsisMenu.setMenuVisibility(false);
      }
    });

    this.legendPostRenderProcess(
      this.chartOptions.legend,
      this.svgSelection,
      true,
      () => {
