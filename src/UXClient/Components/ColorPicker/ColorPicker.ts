...
class ColorPicker extends Component{
    ...
    public render (options: ColorPickerOptions = {}) {
        ...
        d3.select("html").on("click." + this.componentId, (event) => {
            if (this.colorPickerElem.select(".tsi-colorPickerButton").filter(el => el == event.target).empty() && 
                this.colorPickerElem.select(".tsi-colorPickerButton").selectAll("*").filter(el => el == event.target).empty() &&
                this.colorPickerElem.selectAll(".tsi-colorGrid").filter(el => el == event.target).empty()) {
                    this.hideColorGrid();
            }
        });
    }
