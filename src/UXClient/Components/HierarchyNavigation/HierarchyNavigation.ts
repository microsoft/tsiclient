    ...
    public render(environmentFqdn: string, getToken: any, hierarchyNavOptions: any){
        ...
        getToken().then(token => {
            this.server.getTimeseriesHierarchies(token, environmentFqdn).then(r => {
                ...
                //tree
                this.hierarchy = results.append('div').classed('tsi-hierarchy-filtered', true);
                //flat list
                this.instanceList = results.append('div').classed('tsi-hierarchy-instances', true);
