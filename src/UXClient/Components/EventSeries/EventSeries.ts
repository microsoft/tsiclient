	...
	public render(namedData: Array<any>, options: any = {}){
		...
		rects.on("mouseover", function (dRect, iRect) { 
			self.elementMouseover(dRect, iRect, (d, i) => { 
				return Utils.timeFormat(this.usesSeconds, this.usesMillis, self.chartOptions.offsetMinutes)(new Date(d.time));
			});
			...
			tooltip.draw (dRect, {}, mousePos[0], mousePos[1], {top: 0, bottom: 0, left: 0, right: 0}, (text) => {
				text.text(null);
				text.append('div')
					.text(Utils.timeFormat(self.usesSeconds, self.usesMillis, self.chartOptions.offsetMinutes)(new Date(dRect.time)))
					.classed('title', true);
				text.append('div')
