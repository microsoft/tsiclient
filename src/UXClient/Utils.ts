...
class Utils {
    ...
    static TsqRange = class {
        ...
        calculateNeatBucketSize(targetNumberOfBuckets: number) {
            ...
            for (; neatIntervalIndex < Utils.TsqRange.NeatIntervalsMs.length; neatIntervalIndex++) {
                if (Utils.TsqRange.NeatIntervalsMs[neatIntervalIndex] > estimatedBucketSizeMs) { break; }
            }

            this.bucketSizeMs = Utils.TsqRange.NeatIntervalsMs[neatIntervalIndex - 1];
        ...
        get bucketSizeStr() {
            let bucketSize = Utils.TsqRange.millisToLargestUnit(this.bucketSizeMs);
